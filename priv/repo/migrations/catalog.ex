...
def change_category(%Category{} = category, attrs \\ %{}) do
  ...
  defp filter_by_product(query, product_id) do
    where(query, [s], s.product_id == ^String.to_integer(product_id))
  end
  ...
  def create_stock_movement(attrs) do
    changeset = StockMovement.changeset(%StockMovement{}, attrs)

    if changeset.valid? do
      product_id = Ecto.Changeset.get_field(changeset, :product_id)
      tipo = Ecto.Changeset.get_field(changeset, :tipo)
      quantidade = Ecto.Changeset.get_field(changeset, :quantidade)
      ...
      Repo.transaction(fn ->
        ...
        product
        |> Product.changeset(%{estoque: novo_estoque})
        |> Repo.update!()
  ...
  def update_stock_movement(%StockMovement{} = stock_movement, attrs) do
    changeset = StockMovement.changeset(stock_movement, attrs)

    if changeset.valid? do
      novo_tipo = Ecto.Changeset.get_field(changeset, :tipo)
      nova_quantidade = Ecto.Changeset.get_field(changeset, :quantidade)
      novo_product_id = Ecto.Changeset.get_field(changeset, :product_id)

      Repo.transaction(fn ->
        # Produto relacionado à movimentação antiga
        produto_antigo = Repo.get!(Product, stock_movement.product_id)

        # 1. Reverte o efeito da movimentação antiga
        estoque_revertido =
          case stock_movement.tipo do
            "entrada" ->
              produto_antigo.estoque - stock_movement.quantidade
        ...
        produto_antigo
        |> Product.changeset(%{estoque: estoque_revertido})
        |> Repo.update!()

        # Produto relacionado à nova movimentação
        produto_novo =
          if novo_product_id == stock_movement.product_id do
            %{produto_antigo | estoque: estoque_revertido}
          else
            Repo.get!(Product, novo_product_id)
          end

        # 2. Aplica o efeito da nova movimentação
        novo_estoque =
          case novo_tipo do
            "entrada" ->
              produto_novo.estoque + nova_quantidade

            "saida" ->
              if produto_novo.estoque < nova_quantidade do
                Repo.rollback(:estoque_insuficiente)
              end
  ...
  def delete_stock_movement(%StockMovement{} = stock_movement) do
    Repo.transaction(fn ->
      product = Repo.get!(Product, stock_movement.product_id)

      novo_estoque =
        case stock_movement.tipo do
          "entrada" ->
            product.estoque - stock_movement.quantidade
  ...
  def dashboard do
    ...
    produtos_com_estoque =
      Product
      |> where([p], p.ativo == true and p.estoque > 0)
      |> Repo.aggregate(:count, :id)
    ...
    percentual_produtos_ativos =
      if total_produtos == 0 do
        ...
        Float.round(produtos_ativos * 100 / total_produtos, 2)
      end
    ...
    total_movimentacoes =
      Repo.aggregate(StockMovement, :count, :id)
    ...
    inicio_7_dias =
      NaiveDateTime.add(agora, -7 * 24 * 60 * 60, :second)

    inicio_30_dias =
      NaiveDateTime.add(agora, -30 * 24 * 60 * 60, :second)

    movimentacoes_7_dias =
      StockMovement
      |> where([s], s.inserted_at >= ^inicio_7_dias)
      |> Repo.aggregate(:count, :id)

    movimentacoes_30_dias =
      StockMovement
      |> where([s], s.inserted_at >= ^inicio_30_dias)
      |> Repo.aggregate(:count, :id)

    entradas_30_dias =
      StockMovement
      |> where([s], s.tipo == "entrada" and s.inserted_at >= ^inicio_30_dias)
      |> Repo.aggregate(:sum, :quantidade)
      |> Kernel.||(0)

    saidas_30_dias =
      StockMovement
      |> where([s], s.tipo == "saida" and s.inserted_at >= ^inicio_30_dias)
      |> Repo.aggregate(:sum, :quantidade)
      |> Kernel.||(0)
    ...
    taxa_entrada =
      if total_movimentacoes == 0 do
        ...
        Float.round(movimentacoes_entrada * 100 / total_movimentacoes, 2)
      end
    ...
    taxa_saida =
      if total_movimentacoes == 0 do
        ...
        Float.round(movimentacoes_saida * 100 / total_movimentacoes, 2)
      end

    media_entradas_dia =
      Float.round(entradas_30_dias / 30, 2)

    media_saidas_dia =
      Float.round(saidas_30_dias / 30, 2)

    cobertura_estoque_dias =
      if media_saidas_dia == 0 do
        nil
      else
        Float.round(estoque_total / media_saidas_dia, 2)
      end

    valor_total_estoque =
      Product
      |> where([p], p.ativo == true)
      |> select([p], sum(p.estoque * p.preco))
      |> Repo.one()
    ...
    produtos_estoque_baixo =
      Product
      |> where([p], p.ativo == true and p.estoque > 0 and p.estoque <= 5)
    ...
    produtos_parados =
      Product
      |> where([p], p.ativo == true and p.estoque > 0)
      |> where([p], p.id not in subquery(produtos_movimentados_30_dias))
      |> order_by([p], asc: p.nome)
      |> Repo.all()
      |> Repo.preload(:category)
