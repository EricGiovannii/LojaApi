defmodule LojaApi.CatalogTest do
  ...
  def get_stock_movement!(id) do
    case Repo.get(StockMovement, id) do
      ...
      stock_movement ->
        Repo.preload(stock_movement, :product)
    end
  end
  ...
  def create_stock_movement(attrs) do
    ...
    if changeset.valid? do
      ...
      Repo.transaction(fn ->
        ...
        product
        |> Product.estoque_changeset(%{estoque: novo_estoque})
        |> Repo.update!()
  ...
  def update_stock_movement(%StockMovement{} = stock_movement, attrs) do
    ...
    if changeset.valid? do
      ...
      Repo.transaction(fn ->
        produto_antigo =
          case Repo.get(Product, stock_movement.product_id) do
            nil ->
              Repo.rollback(:not_found)
        ...
        produto_antigo
        |> Product.estoque_changeset(%{estoque: estoque_revertido})
        |> Repo.update!()
        ...
        produto_novo =
          if novo_product_id == stock_movement.product_id do
            ...
            case Repo.get(Product, novo_product_id) do
              nil ->
                Repo.rollback(:not_found)
